...
async function setReviewsForBusinessUser(id) {
  let reviewsResp = await getData(
    REVIEW_URL + `?business_user_id=${id}&ordering=${currentReviewOrdering}`,
  );
  if (reviewsResp.ok) {
    currentReviews = reviewsResp.data;
  } else {
    showToastMessage(true, [
      "Bewertung zu diesem User konnte nicht gefunden werden",
    ]);
  }
}
...
async function setReviewsForCustomerUser(id) {
  let reviewsResp = await getData(
    REVIEW_URL + `?reviewer_id=${id}&ordering=${currentReviewOrdering}`,
  );
  if (reviewsResp.ok) {
    currentReviews = reviewsResp.data;
  } else {
...
async function createReview(data) {
  let index = currentReviews.findIndex(
    (item) => item.reviewer === currentUser.user,
  );
  if (index >= 0) {
...
function openReviewEditDialog(reviewId) {
  ...
  if (index < 0) {
    ...
    document.getElementById("rating_dialog").innerHTML =
      getReviewDialogformTemplate(currentReviews[index], true);
  }
}

function openReviewDeleteDialog(reviewId) {
  document.getElementById("rating_dialog").innerHTML =
    getDeleteOrNotTemplate(reviewId);
}
