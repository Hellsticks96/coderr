...
function getOfferDialogTemplate() {
  ...
  let deleteBtn = currentOfferId
    ? '<button onclick="deleteOffer()" type="button" class="std_btn btn_delete pad_s ">Angebot löschen</button>'
    : "";
...
function getOfferDetailDialogTemplateList() {
  if (
    !currentOffer ||
    !Array.isArray(currentOffer.details) ||
    currentOffer.details.length === 0
  ) {
  ...
  for (let index = 0; index < currentOffer.details.length; index++) {
    detailTemplateList += getOfferDetailDialogTemplate(
      currentOffer.details[index],
    );
  }
...
function getOfferDetailDialogTemplate(detail) {
  ...
  let revisionsDisabled =
    detail.revisions == null || detail.revisions >= 0 ? "" : "disabled";
...
function getOfferDetailFeatureTemplateList(detail) {
  ...
  for (
    let featureIndex = 0;
    featureIndex < detail.features.length;
    featureIndex++
  ) {
